...
describe ApplicationController do
  ...
  context 'not authenticated user' do
    ...
    describe 'GET#preview_our_stories' do
      it 'should not allow user access' do
        get :preview_our_stories
        should_deny_user_access
      end
  ...
  describe 'GET #preview_our_stories' do
    ...
    let(:yesterday) { Date.yesterday }
    let(:the_day_before_yesterday) { 2.days.ago.to_date }
    let(:today_story) { create(:story, time_line: today, published: false) }
    let(:yesterday_story) { create(:story, time_line: yesterday, published: false) }
    let(:the_day_before_yesterday_story) { create(:story, time_line: the_day_before_yesterday, published: false) }
    ...
    it 'renders template' do
      today_story
      get :preview_our_stories, date: today

      expect(response).to render_template(:preview_our_stories)
    end

    it 'gets the stories for latest date if pass in an invalid format date' do
      today_story
      get :preview_our_stories, date: 'invalid date'
    ...
    it 'gets stories by date' do
      today_story
      yesterday_story
      get :preview_our_stories, date: yesterday
    ...
    it 'gets stories for latest if stories for specific date not found' do
      ...
      get :preview_our_stories, date: the_day_before_yesterday
